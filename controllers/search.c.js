...
exports.viewAllDoctors = async (req, res, next) => {
    ...
    exports.viewAllNurse = async (req, res, next) => {
        try {
            const rs = await NursesM.getAll();
            let role = "patient";
            if (req.session.Doctor) {
                role = "doctor";
            }
            if (req.session.Username) {
                res.render('search-nurse', { doctors: rs, display1: "d-none", display2: "d-block", role: role });
            }
            else {
                res.render('search-nurse', { doctors: rs, display1: "d-block", display2: "d-none", role: role });
            }
        } catch (err) {
            next(err);
        }
