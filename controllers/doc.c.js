...
exports.createInvoice = async (req, res, next) => {
    ...
    if (!req.session.Doctor) {
        ...
        const nurse = await NursesM.getAll();
        const sv = await ServicesM.getAll();
        var today = new Date();
        ...
        res.render('invoice', {
            patients: patients, doctor: doctor[0], nurse: nurse, sv: sv,
            today: today, drugs: drugs, date: date, time: time, display1: "d-none", display2: "d-block", role: "doctor"
        });
    }
}
...
exports.viewRecordDetail = async (req, res, next) => {
    ...
    try {
        ...
        while (true) {
            ...
            if (rs[0]['Name' + i] != undefined) {
                array.push({
                    Name: rs[0]['DrugName' + i],
                    Unit: rs[0]['DrugUnit' + i],
                    Quantity: rs[0]['DrugQuantity' + i],
                    Price: rs[0]['DrugPrice' + i],
                    Total: rs[0]['DrugTotal' + i]
                });
            }
            else {
                break;
...
exports.CountListDisease = async (req, res) => {
    if (req.session.Doctor) {
        // if(req.role = 'doctor'){
        const { month, year } = req.query; // Get input from HTML form
        console.log(month, year)
        try {
            const data = await RecordsM.CountListDisease(month, year);
            res.render('list-disease', { data }); // Render the report page with the data
        } catch (err) {
            res.status(500).send('Error occurred while fetching data');
        }
    }
    else {
        return res.render('error', { display1: "d-none", display2: "d-block" });
    }
},
    exports.CalculateRevenue = async (req, res) => {
        if (req.session.Doctor) {
            ...
            try {
                ...
                const revenue = await RecordsM.calculateRevenue(month, year);

                // Render the view with the revenue result
                res.render('revenue_report', { revenue, month, year });
            } catch (err) {
                console.error(err);
